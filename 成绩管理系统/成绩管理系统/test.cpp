...
void student::find()
{
	...
	else
	{
		...
		cout << "学号	  姓名  性别   语文  英语  数学  物理  化学  生物  平时成绩 总评\n";
		cout << "---------------------------------------------------------------------\n";
		cout << setw(5) << temp.number << setw(6) << temp.name << setw(6) << temp.sex <<
			setw(6) << temp.score[0] << setw(6) << temp.score[1] << setw(6) << temp.score[2] <<
			setw(6) << temp.score[3] << setw(6) << temp.score[4] << setw(6) << temp.score[5] <<
			setw(6) << temp.score[6] << setw(9) << temp.last << endl;
	}
	getchar();
}
void student::modify()
...
void student::save()
{
	...
	if (!fout)cout << "文件不能被打开！" << endl;
	else
	{
		for (int i = 0; i<n; i++)
		{
			fout << ' ' << stu[i].number << ' ' << stu[i].name << ' ' << stu[i].sex << ' ' << stu[i].score[0] << ' ' << stu[i].score[1]
				<< ' ' << stu[i].score[2] << ' ' << stu[i].score[3] << ' ' << stu[i].score[4] << ' ' << stu[i].score[5] << ' ' << stu[i].score[6]
...
void student::load()
{
	...
	else
	{
		...
		{
			if (fin.eof())break;
			fin >> stu[i].number >> stu[i].name >> stu[i].sex >> stu[i].score[0] >> stu[i].score[1]
				>> stu[i].score[2] >> stu[i].score[3] >> stu[i].score[4] >> stu[i].score[5] >> stu[i].score[6]
				>> stu[i].total >> stu[i].average >> stu[i].last;
		}
		cout << "文件已读取成功" << endl;
...
{
	cout << setw(5) << s2.number << setw(6) << s2.name << setw(6) << s2.sex <<
		setw(6) << s2.score[0] << setw(6) << s2.score[1] << setw(6) << s2.score[2] <<
		setw(6) << s2.score[3] << setw(6) << s2.score[4] << setw(6) << s2.score[5] <<
		setw(6) << s2.score[6] << setw(9) << s2.last << endl;
	return scout;
}
...
void menu()
{
	...
	cout << "\t\t\t\t7. 文件读出数据" << endl;
	cout << "\t\t\t\t0. 退出系统" << endl;
	cout << "\n\n\t请输入一位数字(1-7)选择您想要的功能，并按回车键确认（退出请输入0）：" << endl;
